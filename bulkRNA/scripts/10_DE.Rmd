...
# save table
path <- paste0(out, "DEG_comparison/DEG_comparison_FDRq_", 
...
for (i in allComparisons) {
  ...
  #output as txt file
  path <- paste(out, "DEG_tables/", title, "_FDRq_1.00_LFC_0.00.tsv", sep = "") 
  write.table(
    vTopTableAll,
    path,
    sep = "\t",
  ...
  path <- paste(out, "DEG_tables/", title, "_FDRq_0.05_LFC_0.00.tsv", sep = "") 
  write.table(
    vTopTable1,
    path,
    sep = "\t",
  ...
  path <- paste(out, "DEG_tables/", title, "_FDRq_0.10_LFC_0.00.tsv", sep = "") 
  write.table(
    vTopTable2,
    path,
    sep = "\t",
...
# get file list
files <- list.files(paste0(out, "DEG_tables/"))
keep <- grep("_1.00_LFC_0.00.tsv", files)
files <- files[keep]
files <- paste0(out, "DEG_tables/", files)
...
for (i in files) {
 ...
 if(num != 0) {
    ...
    # save
    path <- paste0(out, "volcano/", comparison, "_FDRq_", 
...
files <- list.files(paste0(out, "DEG_tables/"))
keep <- grep("_1.00_LFC_0.00.tsv", files)
files <- paste0(out, "DEG_tables/", files[keep])
 ...
 down.filename <- paste0(out, "metascape_input/", comparison,
  ...
  if (length(up) > 10) {
      msbio <- jsonlite::toJSON(list(
        input = gsub(paste0(out, "metascape_input/"),
...
write.table(x = msbio.df,
            file = paste0(out, "metascape_input/msbio.job"),
            quote = FALSE,
            row.names = FALSE,
            col.names = FALSE)
```
