...
data <- as.data.frame(table(mouse_filtered$sample))
...
den1 <- ggplot(mouse_filtered@meta.data,
       aes(color = sample,
           x = nCount_RNA,
           fill = sample)) +
...
den2 <- ggplot(mouse_filtered@meta.data,
       aes(color = sample,
           x = nFeature_RNA,
           fill = sample)) +
  geom_density(alpha = 0.2) +
...
den3 <- ggplot(mouse_filtered@meta.data,
       aes(color = sample,
           x = cell_complexity,
           fill = sample)) +
  geom_density(alpha = 0.2) +
...
den4 <- ggplot(mouse_filtered@meta.data,
       aes(color = sample,
           x = percent_mt,
           fill = sample)) +
  geom_density(alpha = 0.2) +
...
den5 <- ggplot(mouse_filtered@meta.data,
       aes(color = sample,
           x = percent_ribo,
           fill = sample)) +
  geom_density(alpha = 0.2) +
...
den6 <- ggplot(mouse_filtered@meta.data,
       aes(color = sample,
           x = percent_hb,
           fill = sample)) +
  geom_density(alpha = 0.2) +
...
s5 <- FeatureScatter(mouse_filtered,
                     feature1 = "nCount_RNA",
                     feature2 = "percent_mt",
                     group.by = 'sample',
                     shuffle = TRUE)
...
b1 <- ggplot(mouse_filtered@meta.data,
       aes(x = sample, 
...
## Top transcripts
```{r top_transcripts}
df <- data.frame(gene_name = rownames(mouse_filtered))
df$rsum <- rowSums(x = mouse_filtered, slot = "counts")
