...
barcodes <- readRDS("../../rObjects/cellbender_passed_barcodes.rds")
keep <- colnames(mouse.annotated) %in% barcodes
mouse.annotated$cellbender_keep <- keep

u3 <- DimPlot(mouse.annotated,
             group.by = "cellbender_keep",
             label = TRUE,
             dims = c(1,2),
             raster = FALSE,
             cols = c("red","lightyellow"),
             shuffle = TRUE)
u3
...
```{r save_cellbender_keep, eval=FALSE, echo=FALSE}
path <- paste0(out, "clustering_QC/cellbender_keep_umap_dim1&2.pdf")
pdf(path, width = 8, height = 6)
u3
dev.off()

path <- paste0(out, "clustering_QC/cellbender_keep_umap_dim2&3.pdf")
pdf(path, width = 8, height = 6)
u4
dev.off()
```
